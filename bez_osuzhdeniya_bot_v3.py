...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = """
👋 <b>Добро пожаловать в БезОсужденияBot!</b>

Это безопасное пространство для обмена историями без осуждения. Все истории публикуются <b>анонимно</b> — никто не узнает, кто их автор.

<b>📝 Как отправить историю:</b>
Просто напишите вашу историю одним сообщением прямо в этот чат. Бот автоматически сохранит её и опубликует в канале для обсуждения.

<b>🔒 Ваша приватность защищена:</b>
• Ваше имя и контакты не публикуются
• Истории анонимны для всех
• Только текст истории виден в канале

<b>💬 Что делать дальше:</b>
Напишите свою историю прямо сейчас!
"""
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("📢 Перейти в канал", url=f"https://t.me/c/{CHANNEL_ID[4:]}/1")]
    ]
...
A: Да! Ваше имя и контакты никогда не публикуются в канале.

<b>Q: Кто увидит мою историю?</b>
A: Подписчики канала смогут прочитать текст истории, но не узнают автора.
...
A: Свяжитесь с администратором канала.
"""
        keyboard = [
            [InlineKeyboardButton("◀️ Назад", callback_data='back_to_start')]
        ]
...
async def list_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    response += "\nИспользуйте /post ID для публикации истории в канал."
...
async def post_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Команда /post ID - опубликовать историю в канал (только для админов)"""
    user_id = update.effective_user.id
    ...
    try:
        ...
        await update.message.reply_text(
            "❌ Не могу отправить в канал. Проверьте CHANNEL_ID и права бота."
        )
        ...
        if AUTO_POST:
            ...
            try:
                ...
                await update.message.reply_text(
                    ...
                    "💬 Ваша история уже в канале. Спасибо за доверие!\n\n"
                ...
                logger.error(f"Ошибка при публикации в канал: {e}")
                await update.message.reply_text(
                    "💌 История принята и сохранена, но не удалось опубликовать в канал. "
